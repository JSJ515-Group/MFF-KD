...
from ultralytics.nn.modules.kernel_warehouse import get_temperature
...
class DetectionDistiller(BaseTrainer):
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        #  dayin
        # print(f"Model nc attribute: {model.yaml['nc']}")
        if weights:
            model.load(weights)
        return model
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    #     pred = self.model.module.predict(batch['img'])  # 使用 model.module 调用 predict 方法
                    # else:
                    #     pred = self.model.predict(batch['img'])  # 普通模型调用 predict 方法
                    #  jieshu
                    with torch.no_grad():
                    ...
                    #  gaide
                    # # 在训练逻辑中调用损失函数 criterion 方法
                    # if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                    #     main_loss, self.loss_items = self.model.module.criterion(pred, batch)  # 使用 model.module 调用 criterion 方法
                    # else:
                    #     main_loss,self.loss_items = self.model.criterion(pred, batch)  # 普通模型调用 criterion 方法
