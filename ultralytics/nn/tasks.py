...
from ultralytics.nn.modules import (AIFI, Add,Zoom_cat,ScalSeq4,ScalSeq,C1, C2, C3, C3TR,SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    Classify, Concat,CBAM, Conv, Conv2, ConvTranspose, Detect,DetectAux, Detect_TADDH,DWConv, DWConvTranspose2d,
                                    Focus, GhostBottleneck, GhostConv, HGBlock, HGStem, Pose, RepC3, RepConv,
                                    RTDETRDecoder, Segment,
                                    EMA,CSPStage,SimFusion_4in,IFM,InjectionMultiSum_Auto_pool,
                                    TopBasicLayer,SimFusion_3in,PyramidPoolAgg,AdvPoolFusion,C2f_EMSC,C2f_MLCA)
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment,DetectAux,Detect_TADDH)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, Pose,DetectAux,Detect_TADDH)):
            s = 256  # 2x min stride
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, DetectAux,Segment,Detect_TADDH):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR,C3Ghost,  nn.ConvTranspose2d, DWConvTranspose2d, C3x, C2f_MLCA,RepC3,CSPStage,C2f_EMSC):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f,C2f_EMSC, C3, C3TR, C3Ghost, C3x, RepC3,CSPStage,C2f_MLCA):
        ...
        #     c2 = sum(ch[x] for x in f)
            ...
            c2 = sum(ch[x] for x in f)
        elif m in (Detect, Segment, Pose,DetectAux,Detect_TADDH):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, DetectAux,Detect_TADDH)):
                return 'detect'
