...
__all__ = 'Detect', 'Segment', 'Pose', 'Classify', 'RTDETRDecoder','Detect_TADDH','DetectAux'
...
class Detect_TADDH(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        super().__init__()
        self.nc = nc  # number of classes
        # hc = [hidc // 4,hidc // 2, hidc, hidc]
        # ch = [hidc,hidc,hidc,hidc]
        self.nl = len(ch)  # number of detection layers
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        ...
        #     nn.Sequential(
        #         Conv_GN(hc[i], hc[i] // 2, 3, 1, 1),
        #         Conv_GN(hc[i] // 2, hc[i] // 2, 3, 1, 1)
        #     ) for i in range(len(hc))
        # ])
        # 使用 AdjustChannel 类来调整所有通道数
        self.adjust_convs = nn.ModuleList([AdjustChannel(channels, hidc) for channels in ch])
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.adjust_convs[i](x[i])
            stack_res_list = [self.share_conv[0](x[i])]
            # stack_res_list = [self.share_conv[i](x[i])]
            # stack_res_list.extend(m(stack_res_list[-1]) for m in self.share_conv[i][1:])
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if hasattr(self, 'dfl_aux'):
                for i in range(self.nl, 2 * self.nl):
                    x[i] = torch.cat((self.cv4[i - self.nl](x[i]), self.cv5[i - self.nl](x[i])), 1)
            y = torch.tensor([32.]).cuda()
            # 使用 torch.cat 连接两个张量
            self.stride = torch.cat((self.stride, y))
            # print('3', self.stride)
