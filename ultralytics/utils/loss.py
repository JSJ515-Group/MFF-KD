...
from .metrics import bbox_iou,bbox_alpha_iou,bbox_mpdiou,wasserstein_loss,bbox_inner_iou
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,mpdiou_hw=None):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, ShapeIoU=True, scale=2.0)
        # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, EIoU=True)
        # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, GIoU=True, ratio=0.7)
        # iou = bbox_mpdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, mpdiou_hw=mpdiou_hw)
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum

        # iou = bbox_alpha_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True,alpha=3)
