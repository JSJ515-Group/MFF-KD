...
# def MPDIoU(box1, box2, xywh=True,eps=1e-7,imgsz=None):
...
#     # IoU
#     iou = inter / union
#     d1_squared = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
#     d2_squared = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
#     mpdiou = (inter / union) - d1_squared / (w ** 2 + h ** 2) + d2_squared / (w ** 2 + h ** 2)
#     return mpdiou
#  ,MPDIoU=True,imgsz=None
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    # d1_squared = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
    # d2_squared = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
    # mpdiou = (inter / union) - d1_squared / (800 ** 2 + 800 ** 2) + d2_squared / (800 ** 2 + 800 ** 2)
    if CIoU or DIoU or GIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
            rho2 = torch.sqrt(((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4)
            #rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
                with torch.no_grad():
                    #alpha = v / (v - iou + (1 + eps))
                    alpha = v / (1.5*v - iou + (1 + eps))
